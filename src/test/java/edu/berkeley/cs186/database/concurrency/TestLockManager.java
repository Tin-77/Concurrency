...
public class TestLockManager {
    ...
    @Test
    @Category(PublicTests.class)
    public void testAcquireReleaseRelock() {
        /**
         * Transaction 0 acquires an S lock on table0
         * Transaction 0 acquires an S lock on table1 and releases its lock on table0
         */
        ...
        runner.run(0, () -> {
            lockman.acquireAndRelease(transactions[0], tables[0], LockType.S, Collections.singletonList(tables[1]));
        });

        // table1 should have no locks on it from any transaction
        assertEquals(Collections.emptyList(), lockman.getLocks(tables[1]));
    ...
    @Test
    @Category(PublicTests.class)
    public void testXXSSXXSS() {

        DeterministicRunner runner = new DeterministicRunner(8);
        runner.run(0, () -> lockman.acquire(transactions[0], dbResource, LockType.X));
        runner.run(1, () -> lockman.acquire(transactions[1], dbResource, LockType.X));
        runner.run(2, () -> lockman.acquire(transactions[2], dbResource, LockType.S));
        runner.run(3, () -> lockman.acquire(transactions[3], dbResource, LockType.S));
        runner.run(4, () -> lockman.acquire(transactions[4], dbResource, LockType.X));
        runner.run(5, () -> lockman.acquire(transactions[5], dbResource, LockType.X));
        runner.run(6, () -> lockman.acquire(transactions[6], dbResource, LockType.S));
        runner.run(7, () -> lockman.acquire(transactions[7], dbResource, LockType.S));
        ...
        // Block checks
        List<Boolean> blocked_status = new ArrayList<>();
        for (int i = 0; i < 8; ++i) { blocked_status.add(i, transactions[i].getBlocked()); }
        assertEquals(Arrays.asList(false, true, true, true, true, true, true, true), blocked_status);
        ...
        blocked_status.clear();
        for (int i = 0; i < 8; ++i) { blocked_status.add(i, transactions[i].getBlocked()); }
        assertEquals(Arrays.asList(false, false, true, true, true, true, true, true), blocked_status);

        runner.run(1, () -> lockman.release(transactions[1], dbResource));

        // Lock check
        ArrayList<Lock> transLock = new ArrayList<Lock>();
        transLock.add(new Lock(dbResource, LockType.S, 2L));
        transLock.add(new Lock(dbResource, LockType.S, 3L));

        assertEquals(transLock, lockman.getLocks(dbResource));

        // Block checks
        blocked_status.clear();
        for (int i = 0; i < 8; ++i) { blocked_status.add(i, transactions[i].getBlocked()); }
        assertEquals(Arrays.asList(false, false, false, false, true, true, true, true), blocked_status);
        ...
        runner.run(3, () -> lockman.release(transactions[3], dbResource));

        // Lock check
        assertEquals(Collections.singletonList(new Lock(dbResource, LockType.X, 4L)),
                lockman.getLocks(dbResource));

        // Block checks
        blocked_status.clear();
        for (int i = 0; i < 8; ++i) { blocked_status.add(i, transactions[i].getBlocked()); }
        assertEquals(Arrays.asList(false, false, false, false, false, true, true, true), blocked_status);


        runner.run(4, () -> lockman.release(transactions[4], dbResource));

        // Lock check
        assertEquals(Collections.singletonList(new Lock(dbResource, LockType.X, 5L)),
                lockman.getLocks(dbResource));

        // Block checks
        blocked_status.clear();
        for (int i = 0; i < 8; ++i) { blocked_status.add(i, transactions[i].getBlocked()); }
        assertEquals(Arrays.asList(false, false, false, false, false, false, true, true), blocked_status);

        runner.run(5, () -> lockman.release(transactions[5], dbResource));

        // Lock check
        ArrayList<Lock> transLock1 = new ArrayList<Lock>();
        transLock1.add(new Lock(dbResource, LockType.S, 6L));
        transLock1.add(new Lock(dbResource, LockType.S, 7L));

        assertEquals(transLock1, lockman.getLocks(dbResource));

        // Block checks
        blocked_status.clear();
        for (int i = 0; i < 8; ++i) { blocked_status.add(i, transactions[i].getBlocked()); }
        assertEquals(Arrays.asList(false, false, false, false, false, false, false, false), blocked_status);
