...
public class LockManager {
    ...
    private class ResourceEntry {
        ...
        public void grantOrUpdateLock(Lock lock) {
            ...
            for (Lock l : transactionLocks.get(transNum)) {
                if (l.name.equals(lock.name)) {
                    if (!l.lockType.equals(lock.lockType)) {
        ...
        private void processQueue() {
            ...
            while (requests.hasNext()) {
                ...
                boolean compatible = checkCompatible(lock.lockType, request.transaction.getTransNum());
                if (compatible) {
                    for (Lock l :request.releasedLocks) {
                        releaseLock(l);
                        request.releasedLocks.remove(l);
                    }
        ...
        synchronized (this) {
            ...
            if (resourceEntry.checkCompatible(lockType, transNum)) {
                ...
                for (ResourceName n : releaseNames) {
                    if (n == name) continue;
                    resourceEntry = getResourceEntry(n);
    ...
    public void promote(TransactionContext transaction, ResourceName name,
        ...
        synchronized (this) {
            ...
            if (!LockType.substitutable(newLockType, resourceEntry.getTransactionLockType(transNum))) {
                throw new InvalidLockException("Not substitutable for the original type");
            }
